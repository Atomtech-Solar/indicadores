-- Overview admin: lista de comissões já definidas (pendente/disponível), ainda não pagas.
...
propostas_nao_pagas_lista AS (
  SELECT
    ...
    i.valor_projeto::numeric AS valor_projeto,
    COALESCE(c.updated_at, c.created_at) AS definido_em
  FROM public.comissoes c
  LEFT JOIN public.usuarios u ON u.id = c.usuario_id
  LEFT JOIN public.indicacoes i ON i.id = c.indicacao_id
  WHERE c.status IN ('pendente', 'disponivel')
...
SELECT jsonb_build_object(
  ...
  'propostasNaoPagasLista', COALESCE((
    SELECT jsonb_agg(
      jsonb_build_object(
        'id', p.id,
        'usuario_nome', p.usuario_nome,
        'indicacao_nome', p.indicacao_nome,
        'valor', p.valor,
        'status', p.status,
        'valor_projeto', p.valor_projeto,
        'definido_em', p.definido_em
      )
      ORDER BY p.definido_em DESC
    )
    FROM propostas_nao_pagas_lista p
  ), '[]'::jsonb),
...
COMMENT ON FUNCTION public.get_admin_overview() IS
  'Métricas admin + comissões pagas + propostas (comissões definidas não pagas).';
